...
export function exportToFile(fileType: 'json' | 'xlsx' | 'csv' | 'yaml', filename: string, data: pdw.CompleteDataset) {
    if (fileType === 'xlsx') return new AsyncExcelTabular().exportTo(data, filename);
    if (fileType === 'json') return new AsyncJson().exportTo(data, filename);
    if (fileType === 'yaml') return new AsyncYaml().exportTo(data, filename);
    if (fileType === 'csv') return new AsyncCSV().exportTo(data, filename);
...
export async function importFromFile(file: File){
    ...
    if(fileType === 'unknown'){
        console.error('Unknown file extension cannot be imported. Accepted file extensions are .json, .csv, .xlsx, and .yaml');
    }
    let reader = new FileReader();
  
    if (file.type === "application/json") {
        reader.onload = (cont: any) => {
            const importedJSON = cont.target.result;
            importRouter('json', importedJSON)
        };
    }
    if (file.type === "application/x-yaml") {
        reader.onload = (cont: any) => {
            const importedYaml = cont.target.result;
            importRouter('yaml', importedYaml)
        };
    }
    if (file.type === "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet") {
      reader.onload = (cont: any) => {
          const importedYaml = cont.target.result;
          importRouter('xlsx', importedYaml)
      };
  }
    if (file.type === "text/csv") {
      reader.onload = (cont: any) => {
          const importedCSV = cont.target.result;
          importRouter('csv', importedCSV)
      };
  }
 ...
 * @returns 
 */
async function importRouter(fileType: 'json' | 'xlsx' | 'csv' | 'yaml', stringifiedContent: string) {
    if (fileType === 'xlsx') return await new AsyncExcelTabular().importFrom(stringifiedContent);
    if (fileType === 'json') return await new AsyncJson().importFrom(stringifiedContent);
    if (fileType === 'yaml') return await new AsyncYaml().importFrom(stringifiedContent);
    if (fileType === 'csv') return await new AsyncCSV().importFrom(stringifiedContent);
...
export class AsyncJson implements pdw.AsyncDataStore {
    exportTo(data: pdw.CompleteDataset, exportName: string) {
        var dataStr =
    ...
    async importFrom(stringifiedJsonCompleteDataset: string): Promise<pdw.CompleteDataset> {
        const file = JSON.parse(stringifiedJsonCompleteDataset);
...
export class AsyncYaml implements pdw.AsyncDataStore {
    ...
    exportTo(data: pdw.CompleteDataset, exportName: string) {
        ...
        var dataStr =
            ...
            encodeURIComponent(YAML.stringify(newData));
        var downloadAnchorNode = document.createElement("a");
        downloadAnchorNode.setAttribute("href", dataStr);
    ...
    async importFrom(stringifiedYaml: string): Promise<pdw.CompleteDataset> {
        const file = YAML.parse(stringifiedYaml);
        ...
        returnData = pdw.PDW.addOverviewToCompleteDataset(returnData);
...
export class AsyncCSV implements pdw.AsyncDataStore {
    //@ts-expect-error
    async importFrom(stingifiedContent: string): pdw.CompleteDataset {
        console.log('loading...');
        let loadedWb = XLSX.read(stingifiedContent, {type: 'string'})
...
export class AsyncExcelTabular implements pdw.AsyncDataStore {
    ...
    async importFrom(stringifiedContent: string): Promise<pdw.CompleteDataset> {
        /**
        ...
        let loadedWb = XLSX.read(stringifiedContent, {type: 'string'});
        // let loadedWb = XLSX.
        const shts = loadedWb.SheetNames;
        const pdwRef = pdw.PDW.getInstance();
    ...
    static parseExcelDef(defRow: any, points: any): pdw.DefLike {
        //check structure
        if (defRow._deleted == undefined || defRow._did == undefined) throw new Error('Cannot parseExcelDefRow ');
        ...
        if (!pdw.Def.isDefData(defData)) throw new Error('Failed to correctly parseExcelDefRow ');
    ...
    static parseExcelPointDefRow(pointDefRow: any): pdw.PointDefLike {
        ...
        if (!pdw.PointDef.isPointDefData(pointDef)) throw new Error('Failed to correctly parseExcelDefRow  ');
    ...
    static parseExcelEntryRow(entryRow: any): pdw.EntryLike {
        ...
        if (entryRow._deleted == undefined
            ...
            throw new Error('Cannot parseExcelEntryRow  ');
        ...
        if (!pdw.Entry.isEntryData(entryRow)) throw new Error('Failed to correctly parseExcelEntryRow ');
